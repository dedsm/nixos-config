...
- [`docs/dms.md`](./docs/dms.md) — DankMaterialShell: what it replaced, how `settings.json` is declared while `session.json` stays mutable, the `dankshell` PAM stack, the `dms ipc` keybind targets, and the panels it must not be allowed to own
