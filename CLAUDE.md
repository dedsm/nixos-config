...
│   │   └── dms/                # DankMaterialShell: package, PAM stack, geoclue entry
...
- **Login/lock flow**: greetd runs DankMaterialShell's greeter (`dedsm.greetd`), which authenticates before the Hyprland session exists and unlocks gnome-keyring on greetd's own PAM stack; plymouth covers the boot so no console text shows between splash, greeter and session — see [`docs/login-flow.md`](./docs/login-flow.md)
- **Fingerprint policy**: `dedsm.fingerprintPolicy` gates *all* fprintd use (the lock screen, sudo, login) from a single polkit rule, implementing Apple's documented Touch ID conditions (restart, logout, 48h idle, password interval + 4h biometric grace, 5 failed matches) — see [`docs/login-flow.md`](./docs/login-flow.md)
- **SSH keys**: Managed through Home Manager SSH configuration
- **GPG setup**: Configured for git signing with a specified key ID; agent enablement is opt-in via the `gnupg` NixOS module on Linux and `programs.gnupg.agent` in `modules/darwin/default.nix` on macOS. The `ssh` home-manager module points `SSH_AUTH_SOCK` at that agent on both platforms and refreshes its pinentry on every activation, because the agent resolves pinentry to a `/nix/store` path once at startup and nothing otherwise recycles it across a rebuild: an agent that outlives a gnupg or pinentry bump fails every passphrase prompt with "No pinentry" once GC drops the old path — while still answering pings and looking healthy — and takes ssh down with it, not just signing. The remedy differs per platform: NixOS keeps the path in `/etc/gnupg/gpg-agent.conf`, so `gpgconf --reload` (SIGHUP) suffices and leaves socket activation and cached keys alone; Darwin has no `gpg-agent.conf` at all and uses gnupg's compiled-in default, so the process itself must be killed and is restarted on demand by the module's zsh hook
...
- [`docs/brain-skill.md`](./docs/brain-skill.md) — the personal "second brain" Claude Code skill, including the v14 state-reading gate (no tracker standings in a page body) and how it is scoped to a commit's added lines
- [`docs/dms.md`](./docs/dms.md) — DankMaterialShell: what it replaced, how `settings.json` is declared while `session.json` stays mutable, the `dankshell` PAM stack, the two patches on the package (a `timeout=` on the bundled fingerprint stack and a lower retry-backoff cap, without which the reader is unclaimed half of every lock), the `dms ipc` keybind targets, and the panels it must not be allowed to own
