...
- **`SessionStart` → `claude-brain-health.sh`**: surfaces `brain health`'s one-line store vitals when a session starts (matcher `startup|resume|clear` — deliberately not `compact`, which would repeat the line mid-task). It emits **JSON**, not plain text, because the flag has two audiences and a hook's plain stdout only reaches one of them: stdout becomes Claude's context, so the user wouldn't see the flag until Claude's first reply. The object carries `systemMessage` (rendered to the user directly, at session start) *and* `hookSpecificOutput.additionalContext` (the context injection), so one run reaches both. `brain health`'s line is arbitrary text, so `jq -n --arg` builds the object rather than string interpolation. Fail-open (always exits 0) and silent — no output at all — when the store is clean or absent. The same check also runs on a weekly timer (systemd user timer / launchd agent) that raises a desktop notification — see [`brain-skill.md`](./brain-skill.md) § Ambient surfacing.
