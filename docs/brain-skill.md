...
3. **A one-time bootstrap** that seeds `~/brain` from the template *only if `~/brain` doesn't exist yet* (`home.activation.bootstrapBrain` in `default.nix`). It copies the template, `chmod -R u+w`s it (Nix store files are read-only), and `git init`s it — then the same activation installs the hooks.
...
This reads the canonical template shipped with the (possibly just-updated) skill, diffs it against the live `~/brain`, and — after showing the diff and asking before anything destructive — updates `~/brain/CLAUDE.md` to the canonical manual, creates missing bucket directories, migrates page frontmatter to the current schema (backfilling `created`/`finished` from git history and `summary` from the existing index where possible), runs `brain normalize` + `brain check` to confirm validity, verifies the commit hooks are present (they are installed by the rebuild that precedes sync, not by sync itself), and regenerates `index.md` with `brain reindex`. It never touches the substance of existing pages.
